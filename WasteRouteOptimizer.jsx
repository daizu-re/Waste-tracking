...
export default function WasteRouteOptimizer() {
  ...
  const comparison = useMemo(() => {
    return Object.keys(ALGO_FN).map((name) => {
      ...
      return {
        name,
        distance: r.distance !== undefined ? r.distance : 0,
  ...
  const singleDistance = singleResult.distance !== undefined ? singleResult.distance : 0;
  ...
  }, [threshold, binsNeedingCollection]);
          ...
          <svg viewBox="0 0 660 400" style={{ width: "100%", height: "auto" }}>
            {EDGES.map(([a, b, w], i) => {
              ...
              return (
                <g key={i}>
                  ...
                  />
                  <text x={(A.x + B.x) / 2} y={(A.y + B.y) / 2 - 4} className="mono" fontSize="9" fill="#5D6459" textAnchor="middle">
                    {w}
                  </text>
                </g>
              );
            ...
            {NODES.map((n) => (
              <g key={n.id}>
                ...
                <text x={n.x} y={n.y + (n.depot ? 30 : n.bin ? 26 : 18)} className="mono" fontSize="9.5" fill="#8A9187" textAnchor="middle">
                  {n.label}
                </text>
              </g>
            ))}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "18px", width: "260px", flexShrink: 0 }}>
          {!showFullRoute ? (
            <div className="panel" style={{ padding: "16px" }}>
              ...
              <div className="mono" style={{ fontSize: "9.5px", color: "#8A9187", marginTop: "6px", lineHeight: 1.5 }}>
                {ALGO_BIGO[algo]}
              </div>
            </div>
          ) : (
          ...
          <div className="panel" style={{ padding: "16px" }}>
            ...
            <MetricRow icon={<Gauge size={13} color="#F4A522" />} label="Nodes visited" value={showFullRoute ? "—" : String(singleResult.visited.length)} />
          </div>
        </div>
      </div>

      <div className="panel" style={{ padding: "16px", marginTop: "18px" }}>
        <div className="hdr" style={{ fontSize: "13px", fontWeight: 600, marginBottom: "10px" }}>
          PERFORMANCE DASHBOARD — DEPOT → {nodeById[targetBin].label.toUpperCase()}
        </div>
...
function Legend({ color, label }) {
  return (
    <div style={{ display: "flex", alignItems: "center", gap: "5px" }}>
      ...
      <span className="mono" style={{ fontSize: "10px", color: "#8A9187" }}>
        {label}
      </span>
    </div>
  );
...
function MetricRow({ icon, label, value }) {
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 0", borderBottom: "1px solid #262C28" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "12px", color: "#8A9187" }}>
        {icon}
        {label}
      </div>
